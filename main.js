...
var SceneA = new Phaser.Class({
    ...
    create: function ()
    {
        ...
        supporter_1 = supporterGroup.create(880, 0, "bukakuma").setDisplaySize(23, 28).setGravityY(-100);// 犬
...
var Ending = new Phaser.Class({
    ...
    create: function ()
    {
        ...
        this.physics.add.collider(player, treasure, (p, c)=>{
            // セリフ表示
            dialog.setAlpha(1).setDisplaySize(180, 25)
            dialog_text.setText('いえのなかをさがしてください!').setAlpha(1)
            endTitle = this.add.text(240, 1150, 'くまからのごほうびです ฅʕ ›ᴥ‹ ʔ', { fontFamily: 'PixelMplus12', fontSize: 30, color: '#FFF' }).setAlpha(0).setOrigin(0.5);
            // タイトル表示
            var tween = this.tweens.add({
                targets: endTitle,
                alpha: 1,
